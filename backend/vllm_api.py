...
def make_api_call(messages: List[BaseMessage]):
...
def generate_code_from_task(origin_requirement, task_details):
    # step 1: 提取出 platform
    platform_res = make_api_call([
        SystemMessage(content="""You are an expert AFSIM code assistant. 以 JSON 格式提取出有多少的 platform
有效 JSON 响应的示例：
```json
{
"planforms"：["redPlane1", "blueTank2", "Su-34", "patriot"]
}```
"""),
        HumanMessage(content=origin_requirement)])
    print(platform_res.content)
    return platform_res
    # step 2: 生成 scenario 代码

    # step 3: 生成 start.txt 代码
